...
def get_main_keyboard():
    ...
    keyboard.add_button("💼 Пассивный доход", color=VkKeyboardColor.PRIMARY)
            ...
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if text == "🏆 топ":
                    response = (
                        ...
                        "│ ₿ /топbtc — Bitcoin\n"
                        "│ 💷 /топфунты — фунты\n"
                        "│ 🏆 /топдостижений — достижения\n"
                        "│ ⭐ /топрепутации — репутация\n"
                        "│ 🏴 /топкланов — кланы\n"
                        "╰──────────────────────╯"
                    )
                    keyboard = get_main_keyboard()
                ...
                if text == "🎁 бонус":
                    ...
                    bonus_amount = random.randint(100000, 1000000)
                    
                    role = get_user_role(user_id, roles)
                    is_worker = role in ["модератор бота", "админ бота", "специальный администратор", "руководитель", "владелец"]
                    worker_bonus = 60000000 if is_worker else 0
                    
                    total_bonus = bonus_amount + worker_bonus
